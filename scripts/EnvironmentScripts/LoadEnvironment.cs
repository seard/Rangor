...
public class LoadEnvironment : MonoBehaviour
{
    ...
    public bool destroyCurrent = false;
        ...
        for (float i = 0; i < blockMap.width * blocksPerPixel; i++) // Iterate over width
        {
            for (float j = 0; j < blockMap.height * blocksPerPixel; j++) // Iterate over height
            {
                for (int k = 0; k < colorCode.Length; k++) // Iterate over all colorCodes
                {
                    ...
                    Color pixelColorRounded = new Color(Mathf.Round(pixelColor.r * 255.0f) / 255.0f, Mathf.Round(pixelColor.g * 255.0f) / 255.0f, Mathf.Round(pixelColor.b * 255.0f) / 255.0f); // 10.0f inte 255.0f
